...
import {
  NasaHeader, mapDispatchToProps, mapStateToProps, loadModuleData,
} from '../../src/components/NasaHeader';
...
describe('NasaHeader should render as expected', () => {
  it('module should render correct JSX', () => {
    ...
    const renderedModule = shallow(<NasaHeader {...props} />);
  ...
  describe('mapDispatchToProps', () => {
    it('should update the browser locale and then reload the language pack', async () => {
      ...
      expect(mockDispatch).toHaveBeenNthCalledWith(2, 'I am loading the language pack for nasa-header and my fallback locale is en-US');
    });
  });
  ...
  describe('loadModuleData', () => {
    ...
    it('should load language pack for nasa-header module ', async () => {
      const langPackAsyncState = await loadModuleData({ store: fakeStore });
      expect(langPackAsyncState).toBe('I am loading the language pack for nasa-header and my fallback locale is en-US');
